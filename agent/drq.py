...
class DrQAgent(AGENTBase):
    ...
    def __init__(self, aug_func, extr_lr, extr_beta, config, **kwargs):
        ...
        total = sum([param.nelement() for param in self.extr.parameters()])
        print("Number of parameter: %.2fM" % (total / 1e6))   
        self.extr_q_optimizer = torch.optim.Adam(self.extr.parameters(),
                                                 lr=extr_lr, betas=(extr_beta, 0.999))
        self.extr_aux_optimizer = torch.optim.Adam(self.extr.parameters(),
                                                 lr=5e-4, betas=(extr_beta, 0.999))      
        self.train()
        self.train_targ()
        self.print_module()
