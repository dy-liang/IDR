...
class CFPredictor(nn.Module):
    def __init__(self, latent_state_dim, act_seq_in_dim, rew_seq_in_dim, hidden_dim,
        ...
        self.predictor = mlp(
            latent_state_dim + omg_seq_out_dim,  # + act_seq_out_dim + omg_seq_out_dim
    ...
    def _get_input(self, h, h_ws):
        # if h_as.ndim == 3:
        #     h = h.unsqueeze(0).expand(h_as.size(0), *h.size())
        # cat = torch.cat([h, h_as], dim=-1)
        if h_ws.ndim == 4:
            h = h.unsqueeze(1).expand(*h_ws.size()[:3], h.size(-1))
        elif h_ws.ndim == 3:
            h = h.unsqueeze(0).expand(h_ws.size(0), *h.size())
        return torch.cat([h, h_ws], dim=-1)
    ...
    def forward(self, latent_state,omega_sequence):
        ...
        input = self._get_input(latent_state, latent_w_seq)
        #latent_state = latent_state.unsqueeze(0).expand(256, *latent_state.size())
