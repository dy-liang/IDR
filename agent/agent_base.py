...
class AGENTBase(object, metaclass=abc.ABCMeta):
    ...
    def augment(self, data, num_aug=2, name=''):
        o, a, r, o2, o3, nd, el = data['obs'], data['act'], data['rew'], data['obs2'],  data['obs3'], data['not_done'], data['env_labels']
        infos = data['infos']
        ...
        batch_size = o.size(0)
        if num_aug > 1:
            o, a, r, o2, o3, nd = o.repeat(num_aug, 1, 1, 1), a.repeat(num_aug, 1), r.repeat(num_aug), o2.repeat(num_aug, 1, 1, 1), o3.repeat(num_aug, 1, 1, 1), nd.repeat(num_aug)
        aug_o, aug_o2, aug_o3 = self.aug_func(o), self.aug_func(o2), self.aug_func(o3)
        aug_o, aug_o2, aug_o3 = _handle_data(aug_o), _handle_data(aug_o2), _handle_data(aug_o3)
        data['aug_o'] = aug_o
        data['aug_o2'] = aug_o2
        data['aug_o3'] = aug_o3
    ...
    @torch.no_grad()
    def select_action(self, obs, deterministic=False, tanh=True, to_numpy=True, Train = True):
        if Train:
    ...
    def update(self, replay_buffer, logger, step, save_log, batch_size=None):
        ...
        data1 = replay_buffer.sample_batch(batch_size) if self.aux_task is None or self.aux_mode == 'pad'\
            else replay_buffer.sample_batch_with_rs(batch_size) #replay_buffer.sample_batch_with_rs1(self.select_action,batch_size) #replay_buffer.sample_batch_with_rs(batch_size) #
        data2 = replay_buffer.sample_batch(batch_size) if self.aux_task is None or self.aux_mode == 'pad'\
            else replay_buffer.sample_batch_with_rs(batch_size) #replay_buffer.sample_batch_with_rs1(self.select_action,batch_size)
    ...
    def auxiliary_update(self, data, logger, step, save_log):
        if self.aux_task is not None:
            ...
            if "fourier" in self.aux_mode:
                loss, opt_dict, info_dict = self.aux_task.update_extr(
                    data, self.extr, self.extr_targ, self.rl.select_action, self.aug_func, step, self.aux_mode)
                opt_dict['opt_e'] = self.extr_q_optimizer
                update_params(opt_dict, loss)
            if "pad" in self.aux_mode:
                loss, opt_dict, info_dict = self.aux_task.update_pad_extr(data, self.aug_func, self.extr)
                opt_dict['opt_e'] = self.extr_q_optimizer
                update_params(opt_dict, loss)
