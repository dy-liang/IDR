...
class ProbabilisticStateModel(nn.Module):

    def __init__(self, encoder_feature_dim, layer_width, announce=True, max_sigma=1e1, min_sigma=1e-4):
        super().__init__()
        ...
        assert(self.max_sigma >= self.min_sigma)
        if announce:
            print("Probabilistic state model chosen.")
    def forward(self, x):
        x = self.fc(x)
        x = self.ln(x)
        x = torch.relu(x)
