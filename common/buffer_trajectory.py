...
class BReplayBuffer(Dataset):
    ...
    def sample_batch_without_reward(self, batch_size=None):
        ...
        batch = dict(
            ...
            obs3 = self.obses[(obs_idxs + self.nstep * 2) % self.capacity].reshape(-1, 9, *self.obses.shape[-2:]),
            obs4 = self.obses[(obs_idxs + self.nstep * 3) % self.capacity].reshape(-1, 9, *self.obses.shape[-2:]),
